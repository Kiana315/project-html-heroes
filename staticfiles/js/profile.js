...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (followButton) {
        console.log("follow button identified");
        followButton.addEventListener('click', async function () {
            console.log("follow button clicked");
            let areFriends = true;
            const relationResponse = await fetch(`/api/user/${selfUsername}/anyRelations/${targetUsername}/`);
            if (!relationResponse.ok) {
                throw new Error('Network response was not ok.');
            }
            ...
            if (areFriends) {
                ...
                await createMessage("FR", "Follow request", targetUsername);
